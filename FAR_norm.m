...
A = cell(1,nstage-1);
for s = 1:nstage-1
%     A{s} = randn(npc(s+1)*nsensor(s+1),npc(s)*nsensor(s)).*0.2;
    [U,S,V] = svd(rand(npc(s+1)*nsensor(s+1),npc(s)*nsensor(s)));
    S(abs(S)>1) = dd;
    A{s} = U*S*V';
end
...
save([resultsource,'setting_far_A',num2str(100*dd),'_true_npc'],'out_MSE','out_MSE_std');
...
save([resultsource,'setting_far_A',num2str(100*dd),'_impact_of_npc'],'prop','out_MSE','out_MSE_std');
end